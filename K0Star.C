...
void K0Star(TString input_file, TString input_V0file, TString ouputfile, int ntrkoff){
	...
	//--------------- V0s ---------------
  	TTree *K0StarTreeOutput = new TTree("K0StarTree","");
	std::vector<float> *K0Star_PtVector   = new std::vector<float>(); K0Star_PtVector->clear();
	std::vector<float> *K0Star_EtaVector  = new std::vector<float>(); K0Star_EtaVector->clear();
	std::vector<float> *K0Star_PhiVector  = new std::vector<float>(); K0Star_PhiVector->clear();
	std::vector<float> *K0Star_MassVector = new std::vector<float>(); K0Star_MassVector->clear();
	std::vector<int> *K0Star_ChargeVector = new std::vector<int>(); K0Star_ChargeVector->clear();
	
	std::vector<int> *K0s_NominalVector = new std::vector<int>(); K0s_NominalVector->clear();
	std::vector<int> *K0s_TightVector = new std::vector<int>(); K0s_TightVector->clear();
	std::vector<int> *K0s_LooseVector = new std::vector<int>(); K0s_LooseVector->clear();
	std::vector<float> *K0s_MisIDEEVector = new std::vector<float>(); K0s_MisIDEEVector->clear();
	std::vector<float> *K0s_MisIDPiProtonVector = new std::vector<float>(); K0s_MisIDPiProtonVector->clear();
	std::vector<float> *K0s_MisIDProtonPiVector = new std::vector<float>(); K0s_MisIDProtonPiVector->clear();
	std::vector<float> *K0s_APAlphaVector = new std::vector<float>(); K0s_APAlphaVector->clear();
	std::vector<float> *K0s_APQTVector = new std::vector<float>(); K0s_APQTVector->clear();
	std::vector<float> *K0s_PtVector = new std::vector<float>(); K0s_PtVector->clear();
	...
	K0StarTreeOutput->Branch("K0Star_pt","vector<float>", &K0Star_PtVector);
	K0StarTreeOutput->Branch("K0Star_eta","vector<float>", &K0Star_EtaVector);
	...
	K0StarTreeOutput->Branch("K0s_misidee","vector<float>", &K0s_MisIDEEVector);
	K0StarTreeOutput->Branch("K0s_misidpipro","vector<float>", &K0s_MisIDPiProtonVector);
	K0StarTreeOutput->Branch("K0s_misidpropi","vector<float>", &K0s_MisIDProtonPiVector);
	K0StarTreeOutput->Branch("K0s_alpha","vector<float>", &K0s_APAlphaVector);
	K0StarTreeOutput->Branch("K0s_qt","vector<float>", &K0s_APQTVector);
	K0StarTreeOutput->Branch("K0s_pt","vector<float>", &K0s_PtVector);
	...
	for(int iEvent = 0; iEvent < nEvents; iEvent++) {
		...
		mult_trackp4 = (float) eventPlaneMultiplicity[106];
    		...
    		if( K0s_pt->size() > 0 && nTracks > 0 ){
  	 		...
  	 		for(int idx = 0; idx < totalsize; idx++){ // 2 loops in one
				...
				if(fabs(K0s_dz2->at(ik0s)) < 1.0)continue;
				...
				float Pmp = (float) K0s_d1M->at(ik0s);

				float Pxn = (float) K0s_d2px->at(ik0s);
				float Pyn = (float) K0s_d2py->at(ik0s);
				float Pzn = (float) K0s_d2pz->at(ik0s);
				float Pmn = (float) K0s_d2M->at(ik0s);
				...
				TVector3 dauvec1(K0s_d1px->at(ik0s), K0s_d1py->at(ik0s), K0s_d1pz->at(ik0s));
				TVector3 dauvec2(K0s_d2px->at(ik0s), K0s_d2py->at(ik0s), K0s_d2pz->at(ik0s));
				...
				float Alpha = (Pp*cosx1-Pn*cosx2)/(Pp*cosx1+Pn*cosx2);
 
				float energyd1e = sqrt(electronMass*electronMass+pvector1.P()*pvector1.P());
				float energyd2e = sqrt(electronMass*electronMass+pvector2.P()*pvector2.P());
				float invmass_ee = sqrt((energyd1e+energyd2e)*(energyd1e+energyd2e)-dauvecsum.Mag2());

				float massd1, massd2, energyd1, energyd2, invmasspiproton, invmassprotonpi;

				massd1 = pimass;
				massd2 = pro_mass;
				energyd1 = sqrt(massd1*massd1+pvector1.P()*pvector1.P());
				energyd2 = sqrt(massd2*massd2+pvector2.P()*pvector2.P());
				invmasspiproton = sqrt((energyd1+energyd2)*(energyd1+energyd2)-dauvecsum.Mag2());
				...
				if(fabs(trackVertexDistanceXYArray[jtrk]/trackVertexDistanceXYErrorArray[jtrk]) >= 5.0) continue;
				...
				if( K0s_3Dagl->at(ik0s) > 0.999 && K0s_3Ddl->at(ik0s) > 5.0 ){ K0s_NominalVector->push_back(1); }else{ K0s_NominalVector->push_back(0); }
				if( K0s_3Dagl->at(ik0s) > 0.9995 && K0s_3Ddl->at(ik0s) > 6.5 && fabs(K0s_dxy1->at(ik0s)) >= 1.2 && fabs(K0s_dxy2->at(ik0s)) >= 1.2 && fabs(K0s_dz1->at(ik0s)) >= 1.2 && fabs(K0s_dz2->at(ik0s)) >= 1.2 ){ K0s_TightVector->push_back(1); }else{ K0s_TightVector->push_back(0); }
				K0s_MisIDEEVector->push_back(invmass_ee);
				K0s_MisIDPiProtonVector->push_back(invmasspiproton);
				K0s_MisIDProtonPiVector->push_back(invmassprotonpi);
				K0s_APAlphaVector->push_back(Alpha);
				K0s_APQTVector->push_back(QT);
				...
				K0Star_PhiVector->push_back(system.Phi());
		...
		K0s_LooseVector->clear();
		K0s_NominalVector->clear();
		K0s_TightVector->clear();
		K0s_MisIDEEVector->clear();
		K0s_MisIDPiProtonVector->clear();
		K0s_MisIDProtonPiVector->clear();
		K0s_APAlphaVector->clear();
		K0s_APQTVector->clear();
		K0s_PtVector->clear();
		K0s_EtaVector->clear();
		K0s_PhiVector->clear();
		...
		K0Star_MassVector->clear();
		K0Star_ChargeVector->clear();	
